...
class KnobCanvas extends React.Component {
	...
	componentDidMount(component) {
        ...
        this.relativeWidth =  this.props.width % 1 !== 0 && this.props.width.indexOf('%');
        this.relativeHeight = this.props.height % 1 !== 0 && this.props.height.indexOf('%');
...
console.log(node);
    ...
    init() {
        if (this.value < this.props.options.min
            || this.value > this.props.options.max) { this.value = this.props.options.min; }

        // this.$.val(this.value);
        this.w2 = this.props.width / 2;
        this.cursorExt = this.props.options.cursor / 100;
        ...
        var s = privates.max(
            String(Math.abs(this.props.options.max)).length,
            String(Math.abs(this.props.options.min)).length,
            2
        ) + 2;
    }
    ...
    angle(value) {
        return (value - this.props.options.min) * this.angleArc / (this.props.options.max - this.props.options.min);
    }
    ...
    arc(value) {
        ...
        return {
            ...
            degree: this.options.flip && !this.props.options.cursor
        };
    }
...
class KnobInput extends React.Component {
	...
	constructor( props ) {
		...
		this.styles = props.options.displayInput ? {
            ...
            border : 0,
            background : 'none',
            font : props.options.fontWeight + ' ' + ((props.width / s) >> 0) + 'px ' + props.options.font,
            textAlign : 'center',
	...
	render() {
		return (
			<input type="text" value={this.props.value} style={this.styles} />
		);
	}	
}
...
export default class ReactJSKnob extends React.Component {
	...
	constructor( props ) {
        ...
        props.options.min = props.options.min !== undefined ? props.options.min : 0;
        props.options.max = props.options.max !== undefined ? props.options.max : 100;
        ...
        props.options.cursor = props.options.cursor === true && 30 || props.options.cursor || 0;
        props.options.inline = props.options.inline || true;
        props.options.stopper = props.options.stopper || true;
        props.options.readOnly = props.options.readOnly || false;
        //end gaurdrails/default options
	}

	render() {
		return (
			<KnobWrapper style={{display:'inline', height: this.props.height + 'px', width: this.props.width + 'px'}} {...this.props}>
				<KnobCanvas height={this.props.height} width={this.props.width} {...this.props} />
